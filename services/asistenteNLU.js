    ...
    function: {
        ...
        parameters: {
            ...
            properties: {
                intent: {
                    type: "string",
                    enum: ["crear_cliente", "registrar_venta", "pagar_fiado", "cambiar_precio", "consultar_gastos", "registrar_gasto", "otro"],
                    description: "Qué quiere hacer el usuario. Si ya había una conversación en curso (ver contexto) y el mensaje es una respuesta a lo que se le preguntó, usá el mismo intent que ya estaba activo. Usá 'cambiar_precio' cuando el usuario quiere actualizar/cambiar el precio del bidón de agua. Usá 'consultar_gastos' cuando el usuario quiere saber cuánto gastó, o pide el detalle/listado de gastos, en una fecha puntual o en un rango de fechas (ej: 'los gastos del miércoles 9 de septiembre', 'dame los gastos entre el martes y el jueves', 'cuánto gasté en los últimos 3 días'). Usá 'registrar_gasto' cuando el usuario quiere cargar/anotar un gasto nuevo (ej: 'gasté 5000 en nafta', 'anotá un gasto de mantenimiento de la moto por 12000'). Los intents 'consultar_gastos' y 'registrar_gasto' solo tienen sentido si 'rol_del_usuario' es 'gabriel'; si el usuario no es 'gabriel' y el mensaje habla de gastos, usá 'otro'."
                },
                ...
                monto: { type: ["number", "null"], description: "Monto en pesos. Para 'registrar_venta', si la entrega se carga por dinero en vez de por cantidad de bidones. Para 'registrar_gasto', el monto del gasto que se está cargando." },
                ...
                categoria_nombre: { type: ["string", "null"], description: "Nombre de la categoría del gasto que se quiere registrar (intent 'registrar_gasto'), ej: 'combustible', 'mantenimiento', 'insumos'. Si la categoría no existe todavía, se crea sola, así que no hace falta que sea una de una lista fija." },
                descripcion: { type: ["string", "null"], description: "Descripción opcional del gasto que se está registrando (intent 'registrar_gasto'). Si el usuario dice explícitamente que no quiere agregar descripción (ninguna, no hace falta, nada, sin descripción), devolvé un string vacío \"\"." },
                fecha_gasto: { type: ["string", "null"], description: "Fecha del gasto que se está registrando (intent 'registrar_gasto'), en formato YYYY-MM-DD, resuelta con el mismo criterio que fecha_inicio/fecha_fin en base a 'fecha_actual'. Si el usuario no menciona ninguna fecha, dejalo en null (se va a usar la fecha de hoy)." },
...
async function interpretarMensaje(texto, estado, rolUsuario) {
    ...
    const systemPrompt =
        ...
        "Si el mensaje no tiene nada que ver con crear un cliente, registrar una venta/entrega, saldar un fiado, cambiar el precio del bidón, consultar gastos o registrar un gasto, y tampoco hay una conversación en curso, usá intent 'otro'. " +
